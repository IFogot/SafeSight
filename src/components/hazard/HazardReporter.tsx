...
export const HazardReporter: React.FC = () => {
  const { t, language, hazardReports, addHazardReport, upvoteHazardReport } = useSafeSight();
  ...
  // Simulated Voice Dictation in active language
  const handleToggleVoiceRecord = () => {
    if (!isRecording) {
      setIsRecording(true);
      soundEngine.playAlertBeep('warning');

      // Sample voice dictation presets per language
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Automatic translation synthesis for safety officers
          ...
          <form onSubmit={handleSubmit} className="space-y-3 text-xs">
            ...
            <div className="pt-1">
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                  isRecording
                    ? 'bg-rose-600 text-white border-rose-500 animate-pulse shadow-lg shadow-rose-600/30'
                    : 'bg-gradient-to-r from-amber-500/20 to-cyan-500/20 border-amber-500/40 text-amber-300 hover:from-amber-500/30 hover:to-cyan-500/30'
                }`}
              >
                {isRecording ? (
                  <>
                    <MicOff className="w-4 h-4" />
                    <span>{t.hazard.recording}</span>
                  </>
                ) : (
                  <>
                    <Mic className="w-4 h-4 text-amber-400" />
                    <span>{t.hazard.voiceReportBtn}</span>
                  </>
                )}
              </button>
            </div>
            ...
            >
              ...
              <span>{t.hazard.submitReport}</span>
            </button>

            {isSubmitted && (
              <div className="p-3 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                <span>Ticket logged and dispatched to Safety Officer dashboard!</span>
              </div>
            )}
          </form>
        </div>
        ...
        <div className="lg:col-span-3 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-200">
              {t.hazard.recentReports} ({hazardReports.length})
            </h3>
            <span className="text-xs text-slate-400 font-mono">
              Live Feed • EEC Safety Dispatch
            </span>
          </div>
          ...
          <div className="space-y-3">
            {hazardReports.map((report) => (
              ...
              >
                ...
                {report.descriptionTranslated && (
                  <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 text-xs space-y-1">
                    <div className="flex items-center justify-between text-[10px] text-cyan-400 font-mono font-bold">
                      ...
                      <button
                        onClick={() =>
                          soundEngine.speakText(report.descriptionTranslated, 'th')
                        }
                ...
                {/* Card Footer with Upvotes & Status */}
